...
namespace KBEngine
{
	...
	*/
	public class PersistentInofs
	{
		...
		{
            object[] args = LuaFramework.Util.CallMethod("KBEngineLua", "GetArgs");
            string clientVersion = (string)args[0];
            string clientScriptVersion = (string)args[1];
            string ip = (string)args[2];
            string port = (string)args[3];
		...
		public bool loadAll()
		{
			...
			if(loginapp_onImportClientMessages.Length > 0 && baseapp_onImportClientMessages.Length > 0)
			{
				try
                {
                    object[] re = LuaFramework.Util.CallMethod("KBEngineLua", "importMessagesFromMemoryStream", new object[]{loginapp_onImportClientMessages,
		...
		public void clearMessageFiles()
		{
            ...
            LuaFramework.Util.CallMethod("KBEngineLua", "resetMessages");
		}
